...
// "아래와 같이 사이즈에 M이 아니라 MM으로 사이즈를 잘못 기입했을 경우 TS 오류가 나면 통과"
...
const _sizeOption = { XS: 1, S: 5, M: 2, L: 2, XL: 4 };
const _totalPrice = _SIZE.reduce(
  (currPrice, size) => currPrice + _sizeOption[size.id] * size.price,
  0
);

const _sizeOption1 = { XS: 1, S: 5, M: 2, L: 2, XL: 4 };
const _totalPrice1 = _SIZE.reduce(
  (currPrice, size) => currPrice + _sizeOption1[size.id] * size.price,
  0
);

const _sizeOption2 = { XS: 2, S: 3, MM: 4, L: 5, XL: 6 };

// 오류 => 통과
// const _totalPrice2 = _SIZE.reduce(
//   (currPrice, size) => currPrice + _sizeOption2[size.id] * size.price,
//   0
// );
