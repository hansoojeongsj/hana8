...
-- 제목(title)과 내용(contents)에 대해 FULLTEXT 인덱스 생성
-- like '%검색어%' 보다 자연어 검색 성능이 훨씬 빠름
...
-- FULLTEXT 인덱스 생성 시 InnoDB가 내부적으로 FTS_DOC_ID 를 생성함
...
select title, contents from Notice where contents like '%조선%';

-- 딱 "조선" 있는 것만, "조선의" "고조선"은 안 나옴 
select * from Notice where match(title, contents) against('조선');

-- 
set global innodb_ft_aux_table = 'testdb/Notice'; -- 여러개 가능!
show variables like 'innodb_ft_aux%';

-- 아래 3가지 해주고 index db를 확인함 
...
-- ex1) Emp 테이블과 동일한 데이터로 TestEmp 테이블을 생성하고,
-- PK는 id (auto_increment)로 정한 후 id의 값을 기준으로 1~99, 100~199, 200 이상을 기준으로 세 개의 partition으로 나눠 보세요.
...
-- 파티셔닝 잘못 건거 제거 
-- ALTER TABLE TestEmp REMOVE PARTITIONING;
...
-- ex3) 다시 id 150인 데이터를 insert 해 보세요. 어느 partition에 들어가나요?(explain으로 확인)
-- 그리고 partition3을 200을 기준으로 다시 나눠보세요.
explain select * from TestEmp where id = 200;
