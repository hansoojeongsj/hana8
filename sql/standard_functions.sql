...
select concat(e.ename, ',', d.dname), concat_ws(',', e.ename, d.dname)
	from Emp e
    right outer join Dept d on e.id = d.captain;
...
-- select id, name, birthdt, gender from Student order by ?;
-- 위 sql문에서 성별(gender)을 '여', '남'으로 표시하시오.

-- 문제만 보고 풀게된 다면, 
select name,
       if(gender = 1, '여', '남') as gender
  from Student;
...
select id, name, birthdt, gender from Student
	order by elt(1, id, name, birthdt);

-- case when 문 사용
select id, name, birthdt, gender, 
	case
		when gender = 1 then '여'
        else '남'
	end as genderkor
from Student;

-- elt 문 사용
-- ELT(gender + 1, '남', '여'), 0+1=1 -> 남자 
select id, name, birthdt, gender, 
...
-- ex) substring_index, replace, 
-- select 'https://www.topician.com/lms/lectures'; 에서 도메인 부분만 추출하시오.
select substring_index(
           substring_index('https://www.topician.com/lms/lectures', '/', 3),
           '/',
           -1
       ) as domain;
-- 
select substring_index(
           replace('https://www.topician.com/lms/lectures', 'https://', ''),
           '/',
           1
       ) as domain;
...
select name,
       concat(
           ...
           '*',
           substring(name, floor(char_length(name) / 2) + 2)
       ) as masked_name
  from Student;
